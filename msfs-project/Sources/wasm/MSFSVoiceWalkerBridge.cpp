// ============================================================================
// MSFSVoiceWalkerBridge — WASM standalone module for MSFS 2024
// ----------------------------------------------------------------------------
...
// ClientData-Area namens "MSFSVoiceWalkerPos". Die Python-Seite abonniert
...
enum {
    ...
    CD_AREA_ID          = 0x56574C4B,  // 'VWLK' — MSFSVoiceWalker
    CD_DEF_ID           = 0x56574C4B,
};

static const char* CD_NAME = "MSFSVoiceWalkerPos";
...
static void resolve_ids() {
    ...
    std::printf("[MSFSVoiceWalker] resolved IDs: "
...
static bool init_client_data() {
    ...
    std::printf("[MSFSVoiceWalker] MapClientDataNameToID hr=0x%lx\n", (long)hr);
    ...
    std::printf("[MSFSVoiceWalker] CreateClientData sz=%u hr=0x%lx\n",
    ...
    std::printf("[MSFSVoiceWalker] AddToClientDataDefinition hr=0x%lx\n", (long)hr);
    if (hr != S_OK) return false;
...
static void fire_probe() {
    ...
    if (!s_loggedFirstSet && hr == S_OK) {
        ...
        std::printf(
            "[MSFSVoiceWalker] first SetClientData OK — sz=%u bytes\n",
...
void CALLBACK MsfsVWDispatch(SIMCONNECT_RECV* pData, DWORD /*cb*/, void* /*ctx*/) {
    ...
    if (pData->dwID == SIMCONNECT_RECV_ID_EVENT) {
        ...
        if (!s_periodicStarted) {
            std::printf(
                "[MSFSVoiceWalker] EVENT id=%u → retry periodic\n",
...
static void start_periodic_request() {
    ...
    HRESULT hr = SimConnect_RequestDataOnSimObject(
        ...
        0, 0, 0);
    std::printf(
        "[MSFSVoiceWalker] start_periodic_request hr=0x%lx\n", (long)hr);
    if (hr == S_OK) s_periodicStarted = true;
}

// ----------------------------------------------------------------------------
extern "C" MSFS_CALLBACK void module_init(void) {
    HRESULT hr = SimConnect_Open(&s_simConnect, "MSFSVoiceWalkerBridge",
                                  nullptr, 0, 0, 0);
    if (hr != S_OK || !s_simConnect) {
        std::printf(
            "[MSFSVoiceWalker] SimConnect_Open failed: %ld\n", (long)hr);
        return;
    }
    ...
    std::printf(
        "[MSFSVoiceWalker] AddToDataDefinition hr=0x%lx\n", (long)hr);
    ...
    std::printf(
        "[MSFSVoiceWalker] CallDispatch hr=0x%lx\n", (long)hr);

    std::printf("[MSFSVoiceWalker] module_init OK — ClientData ready\n");
}
