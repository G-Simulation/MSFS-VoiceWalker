# VoiceWalker — Installer-Projekt

Ein **WiX-Toolset-v7**-Projekt, das aus dem Python-Quellcode eine fertige
`VoiceWalker-Setup.msi` erzeugt — **mit einem einzigen Rechtsklick →
...
1. `VoiceWalker.sln` in Visual Studio öffnen.
2. Im Solution Explorer auf **VoiceWalker.Installer** rechtsklicken →
   **Erstellen**.
3. Warten.
...
| 2       | **BuildPythonExes**: PyInstaller baut `dist\VoiceWalker.exe` und `dist\VoiceWalker-Setup.exe` |
| 3       | **GenerateLicenseRtf**: konvertiert `LICENSE` zu `installer\License.rtf`     |
| 4       | **WiX Link**: packt alles in `installer\bin\x64\Release\VoiceWalker-Setup.msi` |
...
```bat
dotnet build installer\VoiceWalker.Installer.wixproj -c Release -p:Platform=x64
```
...
- **Installationsziel**: `%LOCALAPPDATA%\VoiceWalker\` (Per-User, keine
  Admin-Rechte nötig).
- **Installierte Dateien**: `VoiceWalker.exe`, `VoiceWalker-Integrator.exe`
...
- **MSFS-Integration**: der Integrator wird als Custom Action am Ende des
  ...
  `VoiceWalker` in die `exe.xml` ein (mit `.bak`-Backup).
- **Uninstaller**: Windows-Standard; "Apps & Features" → VoiceWalker →
...
| Datei                              | Zweck                                                 |
|------------------------------------|-------------------------------------------------------|
| `VoiceWalker.Installer.wixproj`| MSBuild-Projekt mit allen Build-Steps                 |
...
| `error LGHT0103: cannot find ..\dist\VoiceWalker.exe`            | PyInstaller-Schritt ist übersprungen. Einmal "Rebuild" statt "Build". |
